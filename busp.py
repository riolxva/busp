...
white = [ 1749290548, 706686986, 1820132315, 1389182288, 752618557, 1816422993 ] 
            #я          саша       марго      ильягаранин   мать       ёмиёри
...
async def echo_handler(message: Message) -> None:
    if message.from_user.id in white:
        ...
        w = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🎫 Предъявить билет", web_app=WebAppInfo(url=f"https://czsygg-89-22-177-128.ru.tuna.am?perevoz={response.json()['basicTripInfo']['carrierName']}&route={response.json()['basicTripInfo']['routeName']}&govno={response.json()['basicTripInfo']['vehicleGovNumber']}&cost={response.json()['tariffs'][0]['tariffValueCent']//100*mul}&date={time.day}&hour={time.hour}&min={time.minute}&count={mul}"))]
            ])
