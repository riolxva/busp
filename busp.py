...
def asd(a):
    if a.json()['basicTripInfo']['vehicleTypeName'] == "Автобус":
        ...
        return "🚃 "
def rint():
    x = randint(7000000000,7999999999)
    return x 
...
async def echo_handler(message: Message) -> None:
    ...
    response = requests.post(f"https://buspaybot.icom24.ru/api/search/qr?botName=buspaybot&scannedCode={message.text}", json=data)  
    time = datetime.now() + timedelta(hours=7)
    w = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🎫 Предъявить билет", web_app=WebAppInfo(url=f"https://1z71kp-89-22-177-128.ru.tuna.am?perevoz={response.json()['basicTripInfo']['carrierName']}&route={response.json()['basicTripInfo']['routeName']}&govno={response.json()['basicTripInfo']['vehicleGovNumber']}&cost={response.json()['tariffs'][0]['tariffValueCent']//100}&date={time.day}&hour={time.hour}&min={time.minute}"))]
        ])
    await message.answer(f"Билет куплен успешно.\n{response.json()['basicTripInfo']['carrierName']}\n🚏 {response.json()['basicTripInfo']['routeName']}\n{asd(response)}{response.json()['basicTripInfo']['vehicleGovNumber']}\n🪙 Тариф: Полный {response.json()['tariffs'][0]['tariffValueCent']//100},00 ₽\n🎫 Билет №{rint()}", reply_markup=w)
